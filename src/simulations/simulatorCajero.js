...
import { pruebaChiCuadrado } from "../Estadistica/chiCuadrado.js";

export function simularCajero(
  uniformes,
  replicas = 500,
  lambdaLlegadas = 10,
  lambdaServicio = 1 / 5
  ...
  for (let r = 0; r < replicas; r++) {
    const { clientes, nextIdx } = poisson(
      lambdaLlegadas,
      uniformes,
      idxPoisson
    );
    ...
    const tiempoPromedioServicio =
      tiemposServicio.length > 0
        ? tiempoTotalServicio / tiemposServicio.length
        : 0;
    ...
    ClientesR.push(clientes);
  }

  const mediaClientes =
    resultados.reduce((a, r) => a + r.clientes, 0) / replicas;
  const varClientes =
    resultados.reduce(
      (a, r) => a + Math.pow(r.clientes - mediaClientes, 2),
      0
    ) /
    (replicas - 1);
  const desvClientes = Math.sqrt(varClientes);
  const mediaTiempoServicio =
    resultados.reduce((a, r) => a + r.tiempoPromedioServicio, 0) / replicas;
  ...
  return [
    mediaClientes.toFixed(2),
    varClientes.toFixed(2),
    desvClientes.toFixed(2),
    mediaTiempoServicio.toFixed(2),
    tiempoPromedioPorReplica,
    ClientesR,
    [ksTiempos, chiTiempos],
  ];
}
